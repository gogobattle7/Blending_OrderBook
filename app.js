...
app.route('/api/coins')
    ...
    .post((req, res) => {
        ...
        if (name && !coins.includes(name)) {
            coins.push(name);
            res.status(201).json({ message: '코인 이름이 성공적으로 추가되었습니다.' });
        } else {
            res.status(400).json({ message: '잘못된 코인 이름 또는 이미 존재하는 코인 이름입니다.' });
        }
    });
// 웹 소켓을 통해 오더북 정보를 전달
...
const matchOrders = () => {
    ...
    for (let buyIndex = 0; buyIndex < buyOrders.length; buyIndex++) {
        for (let sellIndex = 0; sellIndex < sellOrders.length; sellIndex++) {
            ...
            if (buyOrder.name === sellOrder.name && buyOrder.price == sellOrder.price) {
                // 체결된 거래 정보 생성
                const transaction = {
                    coin: buyOrder.name,
...
app.get('/api/order_book', (req, res) => {
    ...
    const filteredBuyOrders = buyOrders.filter(order => order.name === coinName);
    // 판매 주문 목록에서 요청된 코인에 해당하는 주문을 필터링
    const filteredSellOrders = sellOrders.filter(order => order.name === coinName);
